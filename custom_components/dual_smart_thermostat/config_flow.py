...
from .schemas import (
    ...
    get_floor_heating_toggle_schema,
    get_grouped_schema,
...
class ConfigFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_basic(
        ...
        if system_type != SYSTEM_TYPE_AC_ONLY:
            ...
            grouped = get_grouped_schema(SYSTEM_TYPE_AC_ONLY, show_heater=False)

        base = get_base_schema()
        # Merge base fields (name, sensor) with grouped schema fields
    ...
    async def async_step_floor_heating(
        ...
        return self.async_show_form(
            step_id="floor_heating",
            data_schema=schema,
            errors=errors,
        )

    async def async_step_floor_heating_toggle(
        self, user_input: dict[str, Any] | None = None
    ) -> FlowResult:
        """Handle floor heating toggle configuration."""
        if user_input is not None:
            self.collected_config.update(user_input)
            return await self._determine_next_step()

        return self.async_show_form(
            step_id="floor_heating_toggle",
            data_schema=get_floor_heating_toggle_schema(),
        )
    ...
    async def _determine_next_step(self) -> FlowResult:
        ...
        if (
            "floor_heating_toggle_shown" not in self.collected_config
            and system_type not in ["ac_only"]
            and system_type
            != "floor_heating"  # floor_heating type already has floor config
        ):
            self.collected_config["floor_heating_toggle_shown"] = True
            return await self.async_step_floor_heating_toggle()

        # Show floor heating config if toggle was enabled
        if (
            self.collected_config.get("enable_floor_heating")
            and CONF_FLOOR_SENSOR not in self.collected_config
        ):
            return await self.async_step_floor_config()

        # Show openings toggle for all systems except AC-only (when not already configured)
        # AC-only systems will handle openings in the combined features step
        system_type = self.collected_config.get("system_type")
        if (
            "openings_toggle_shown" not in self.collected_config
            and system_type != "ac_only"
        ):
            self.collected_config["openings_toggle_shown"] = True
            return await self.async_step_openings_toggle()

        # Show openings selection and config if toggle was enabled
        if (
