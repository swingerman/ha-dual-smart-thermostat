...
async def test_cool_tolerance_activates_cooling_at_correct_threshold(
    hass: HomeAssistant, setup_comp_1  # noqa: F811
) -> None:
    """Test cool_tolerance activates cooling at correct threshold.

    This test verifies that mode-specific cool_tolerance is used
    for activation decisions in COOL mode.
    """
    cooler_switch = "input_boolean.test"
    ...
    # Configure with cool_tolerance=2.0 (mode-specific)
    assert await async_setup_component(
        hass,
        CLIMATE,
        {
    ...
    # Set current temp to 23.9°C
    # Should NOT activate yet (23.9 < 22 + 2.0 = 24.0)
    setup_sensor(hass, 23.9)
    await hass.async_block_till_done()

    assert hass.states.get(cooler_switch).state == STATE_OFF

    # Set current temp to 24.1°C
    # Should activate cooler (24.1 >= 22 + 2.0 = 24.0)
    setup_sensor(hass, 24.1)
    await hass.async_block_till_done()

    assert hass.states.get(cooler_switch).state == STATE_ON


async def test_cool_tolerance_deactivates_cooling_at_correct_threshold(
    hass: HomeAssistant, setup_comp_1  # noqa: F811
) -> None:
    """Test cool_tolerance deactivates cooling at correct threshold.

    This test verifies that mode-specific cool_tolerance is used
    for deactivation decisions in COOL mode.
    """
    cooler_switch = "input_boolean.test"
    ...
    # Configure with cool_tolerance=2.0
    assert await async_setup_component(
        hass,
        CLIMATE,
        {
    ...
    # Start with cooler ON at high temp
    setup_sensor(hass, 25)
    await hass.async_block_till_done()
    assert hass.states.get(cooler_switch).state == STATE_ON

    # Set current temp to 20.1°C
    # Should NOT deactivate yet (20.1 > 22 - 2.0 = 20.0)
    setup_sensor(hass, 20.1)
    await hass.async_block_till_done()

    assert hass.states.get(cooler_switch).state == STATE_ON

    # Set current temp to 19.9°C
    # Should deactivate (19.9 <= 22 - 2.0 = 20.0)
    setup_sensor(hass, 19.9)
    await hass.async_block_till_done()
