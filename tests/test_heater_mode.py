...
async def test_heat_tolerance_activates_heating_at_correct_threshold(
    hass: HomeAssistant, setup_comp_1  # noqa: F811
) -> None:
    """Test heat_tolerance activates heating at correct threshold.

    This test verifies that mode-specific heat_tolerance is used
    for activation decisions in HEAT mode.
    """
    heater_switch = "input_boolean.test"
    ...
    # Configure with heat_tolerance=0.3 (mode-specific)
    assert await async_setup_component(
        hass,
        CLIMATE,
        {
    ...
    # Set current temp to 19.6°C (below target - 0.3)
    # Should activate heater (19.6 <= 20 - 0.3 = 19.7)
    setup_sensor(hass, 19.6)
    await hass.async_block_till_done()

    assert hass.states.get(heater_switch).state == STATE_ON

    # Set current temp to 19.8°C
    # Should NOT activate heater yet (19.8 > 20 - 0.3 = 19.7)
    setup_sensor(hass, 19.8)
    await hass.async_block_till_done()
    # Turn off heater first to test that it doesn't turn back on
    await hass.services.async_call(
        "input_boolean", "turn_off", {"entity_id": heater_switch}, blocking=True
    )
    await hass.async_block_till_done()
...
async def test_heat_tolerance_deactivates_heating_at_correct_threshold(
    hass: HomeAssistant, setup_comp_1  # noqa: F811
) -> None:
    """Test heat_tolerance deactivates heating at correct threshold.

    This test verifies that mode-specific heat_tolerance is used
    for deactivation decisions in HEAT mode.
    """
    heater_switch = "input_boolean.test"
    ...
    # Configure with heat_tolerance=0.3
    assert await async_setup_component(
        hass,
        CLIMATE,
        {
    ...
    # Start with heater ON at low temp
    setup_sensor(hass, 19.5)
    await hass.async_block_till_done()
    assert hass.states.get(heater_switch).state == STATE_ON

    # Set current temp to 20.2°C
    # Should NOT deactivate yet (20.2 < 20 + 0.3 = 20.3)
    setup_sensor(hass, 20.2)
    await hass.async_block_till_done()

    assert hass.states.get(heater_switch).state == STATE_ON

    # Set current temp to 20.4°C
    # Should deactivate (20.4 >= 20 + 0.3 = 20.3)
    setup_sensor(hass, 20.4)
    await hass.async_block_till_done()
